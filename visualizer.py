...
def plot_pm_with_history(
    ...
    """
    绘制包含历史数据与两个预测序列的极移对比图。
    
    参数：
        history: np.ndarray, shape (Lh, 2)，历史数据
        predict1: np.ndarray, shape (Lp1, 2)，预测数据1（蓝色实线）
        predict2: np.ndarray, shape (Lp2, 2)，预测数据2（红色虚线）
        date: datetime 或 str，预测起始日期（历史序列终点为 date-1）
        legend_labels: (str, str, str)，三条曲线的图例名称，如 ("History", "Model A", "Model B")
        date_format: 日期格式，当 date 为 str 时使用
    """
    ...
    # 使用 constrained_layout=True，自动优化边距
    ...
    axes[0].set_ylabel("PMX (as)", labelpad=0)  # labelpad 拉开距离
    axes[0].legend()
    axes[0].grid(True, linestyle="--", alpha=0.5)
    ...
    axes[1].set_ylabel("PMY (as)", labelpad=0)  # labelpad 同样加一点间距
